...
from src.services.interfaces import Telephony
...
class PbxCallRecord(Telephony):
    ...
    def _get_access_token(self):
        ...
        if response.status_code != 200:
            self._logger.error(f"Ошибка получения данных. Статус код ответа: {response.status_code}")
            ...
            self._logger.info(f"Токен PBX создан.")
            return f"{key_id}:{key}"
    ...
    def _get_mp3(self):
        ...
        if uuid_response.status_code == 200:
            if not uuid_response.json()['data']:
                self._logger.error(f"Нет данных. Статус код ответа: {uuid_response.status_code}")
            ...
            self._logger.error(
                f"ERROR: Ошибка получения данных. Статус код ответа: {uuid_response.status_code}. Телефония: pbx")
