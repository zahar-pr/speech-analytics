...
@use_logging(log_file='amocrm') #add self._logger dynamic
class AmoLead(CRM):
    ...
    def _get_lead_main_contact_id(self, lead_id: int) -> int | None:
        ...
        if response.status_code == 200:
            ...
            self._logger.info(f"Основной контакт для сделки найден.CRM: Amo")
    ...
    def _get_first_phone(self):
        ...
        self._logger.info(f"Телефон получен.CRM: Amo")
        return phones_list
    ...
    def post_note(self, note: str) -> None:
        ...
        if response.status_code != 200:
            ...
            self._logger.info(f"Заметка добавлена. Статус код ответа: {response.status_code}.CRM: Amo")
