...
class Bitrix24Lead(CRM):
    ...
    def _get_lead_main_contact_id(self) -> None:
        ...
        if response.status_code == 200 and response_dict.get('error', True):
            ...
            logger.info('ID контакта поулчен.CRM: Bitrix24')
            logger.info('Номер телефона контакта получен.CRM: Bitrix24')
        else:
            logger.error(
    ...
    def post_note(self, note: str) -> None:
        ...
        if response_dict.get('error', None):
            logger.error(f"Ошибка создания заметки.Ошибка: {response_dict.get('error_description')}.Статуск код ошибки: {response.status_code}.CRM: Bitrix24")
            raise Exception("Bitrix24 post note request error")
        else:
            logger.info(f"Заметка создана. ID заметки:{response_dict['result']}.CRM: Bitrix24")